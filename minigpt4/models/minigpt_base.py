...
class MiniGPTBase(BaseModel):
    ...
    # 构造函数里初始化了visual_encoder、ln_vision、llm_backbone、llm_tokenizer
    def __init__(
        self,
        ...
        self.llm_model, self.llm_tokenizer = self.init_llm(
            llm_type=llm_type,
            llm_backbone_path=llm_backbone,
    ...
    def prompt_wrap(self, img_embeds, atts_img, prompts, lengths=None):
        if prompts is None or len(prompts) == 0:
            ...
            pad_emb = self.embed_tokens(torch.tensor(self.llm_tokenizer.pad_token_id, device=img_embeds.device))
    ...
    def forward(self, samples, reduction='mean'):
        ...
        #         return {"loss": loss}
        # todo：修改——不允许使用reduction关键字参数，直接对求未经简化的损失值手动调整
        with self.maybe_autocast():
            # 移除reduction参数，调用llm_backbone
            outputs = self.llm_model(
                inputs_embeds=inputs_embeds,
                attention_mask=attention_mask,
                return_dict=True,
            ...
            if reduction == 'mean':
                ...
                # 计算损失值的总和
                loss = torch.sum(loss)
        # 如果reduction是'none'，则不做任何操作，保持loss为原始损失值向量
