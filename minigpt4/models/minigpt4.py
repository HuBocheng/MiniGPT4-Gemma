...
class MiniGPT4(MiniGPTBase):
    ...
    def __init__(
        ...
        self.llama_proj = nn.Linear(
            img_f_dim, self.llm_model.config.hidden_size
        )
    ...
    def init_Qformer(cls, num_query_token, vision_width, freeze):
        ...
        # 是否冻结Qformer，是的话设置Qformer的参数不需要梯度
        if freeze:
            for name, param in Qformer.named_parameters():
                param.requires_grad = False
            Qformer = Qformer.eval()  # 设置为评估模式，todo ！！！意味着不需要进一步训练模型
    ...
    def from_config(cls, cfg):
        ...
        # 新增：识别模型type
        llm_type = cfg.get("llm_type", None)
        print("llm_type is {}".format(llm_type))
